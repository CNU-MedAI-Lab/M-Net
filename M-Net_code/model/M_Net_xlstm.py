...
class LSTM(nn.Module):
    def __init__(self, length, hidden_size, num_layers):
        super(LSTM, self).__init__()
        # normal input shape is [b, l, d]
    ...
    def forward(self, x):
        # print(x.shape)
        # return self.ln1(self.xlstm_stack(x))
...
class SeqLSTM(nn.Module):
    def __init__(self, length, hidden_size, num_layers):
        ...
        # need input shape is [b(channel), seq_len(f), size(hxw)]
        self.lstm = LSTM(length, hidden_size, num_layers)
        # self.bn1 = nn.BatchNorm2d(length)
...
class ChannelLSTM(nn.Module):
    def __init__(self, length, hidden_size, num_layers):
        ...
        # need input shape is [b(f), seq_len(c), size(hxw)]
        self.lstm = LSTM(length, hidden_size, num_layers)
        # self.bn1 = nn.BatchNorm2d(length)
...
class VSSM(nn.Module):
    def __init__(self, Frames, patch_size=4, in_chans=4, num_classes=3, depths=[2, 2, 9, 2],
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer <= 2:
                if i_layer <2:
                    ...
                    size = 5
                lstm_s1 = SeqLSTM(hidden_size=size * size, length=15, num_layers=3)
                lstm_s2 = SeqLSTM(hidden_size=size * size, length=15, num_layers=3)
                ...
                lstm_c1 = ChannelLSTM(hidden_size=size * size, length=int((dims[0] * 2 ** i_layer)*2), num_layers=3)
                lstm_c2 = ChannelLSTM(hidden_size=size * size, length=int((dims[0] * 2 ** i_layer)*2), num_layers=3)
                ...
                lstm_s1 = SeqLSTM(hidden_size=5*5, length=15, num_layers=3)
                lstm_s2 = SeqLSTM(hidden_size=5*5, length=15, num_layers=3)
                # lstm_s3 = SeqLSTM(input_size=5 * 5, hidden_size=15, num_layers=4)
                # lstm_s4 = SeqLSTM(input_size=5 * 5, hidden_size=15, num_layers=8)

                # lstm_c1 = ChannelLSTM(input_size=5 * 5, hidden_size=int(768/2), num_layers=2)
                lstm_c1 = ChannelLSTM(hidden_size=5*5, length=768, num_layers=3)
                lstm_c2 = ChannelLSTM(hidden_size=5*5, length=768, num_layers=3)
        ...
        self.seq_lstm = SeqLSTM(hidden_size=5*5, length=15, num_layers=3)
        self.channel_lstm = ChannelLSTM(hidden_size=5*5, length=768, num_layers=3)
