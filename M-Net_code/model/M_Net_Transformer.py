...
class SeqViT(nn.Module):
    def __init__(self, size, hidden_size, no_size):
        super(SeqViT, self).__init__()
        # need input shape is [c, size(hxw), seq_len(f)]
        # hidden_size is seq_len, no_size is c
        # self.ViT = Encoder(hidden_size, vis)
        self.pos_embed = nn.Parameter(torch.zeros(no_size, size, hidden_size))

        # Transformer blocks
        self.transformer = nn.ModuleList([
            nn.TransformerEncoderLayer(hidden_size, 5, dim_feedforward=int(hidden_size * 2))
            for _ in range(6)
        ])
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        f, h, w, c = x.shape
        x = x.permute(3, 1, 2, 0) # (c, h, w, f)
        x = x.view(x.shape[0], x.shape[1]*x.shape[2], x.shape[3]) # c, hxw, f
        out = x+self.pos_embed
        for layer in self.transformer:
            out = layer(out)

        x = out + x # c, hxw, f
        x = x.view(x.shape[0], h, w, x.shape[2]) # c, h, w, f
        x = x.permute(3, 1, 2, 0) # f, h, w, c
        return x


class ChannelViT(nn.Module):
    def __init__(self, size, hidden_size, no_size):
        super(ChannelViT, self).__init__()
        # need input shape is [f, size(hxw), seq_len(c)]
        # hidden_size is seq_len, no_size is f
        # self.ViT = Encoder(hidden_size, vis)
        self.pos_embed = nn.Parameter(torch.zeros(no_size, size, hidden_size))

        # Transformer blocks
        self.transformer = nn.ModuleList([
            nn.TransformerEncoderLayer(hidden_size, 6, dim_feedforward=int(hidden_size * 2))
            for _ in range(6)
        ])
    ...
    def forward(self, x):
        ...
        f, h, w, c = x.shape
        # x = x.permute(3, 1, 2, 0)  # (c, h, w, f)
        x = x.view(x.shape[0], x.shape[1]*x.shape[2], x.shape[3])  # f, hxw, c
        out = x + self.pos_embed
        for layer in self.transformer:
            out = layer(out)

        x = out + x  # c, hxw, f
        x = x.view(x.shape[0], h, w, x.shape[2])  # f, h, w, c
        # x = x.permute(3, 1, 2, 0)  # f, h, w, c
        return x
...
class VSSM(nn.Module):
    def __init__(self, Frames, patch_size=4, in_chans=4, num_classes=3, depths=[2, 2, 9, 2],
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer <= 2:
                if i_layer <2:
                    ...
                    size = 5
                lstm_s1 = SeqViT(size=size * size, hidden_size=15, no_size=int((dims[0] * 2 ** i_layer)*2))
                # lstm_s2 = SeqViT(size=size * size, hidden_size=15, no_size=int((dims[0] * 2 ** i_layer)*2))
                ...
                lstm_c1 = ChannelViT(size=size*size, hidden_size=int((dims[0] * 2 ** i_layer)*2), no_size=15)
                # lstm_c2 = ChannelViT(size=size*size, hidden_size=int((dims[0] * 2 ** i_layer)*2), no_size=15)
                ...
                lstm_s1 = SeqViT(size=5*5, hidden_size=15, no_size=768)
                lstm_s2 = SeqViT(size=5*5, hidden_size=15, no_size=768)
                # lstm_s3 = SeqLSTM(input_size=size * size, hidden_size=15, num_layers=4)
                # lstm_s4 = SeqLSTM(input_size=size * size, hidden_size=15, num_layers=8)

                lstm_c1 = ChannelViT(size=5*5, hidden_size=768, no_size=15)
                lstm_c2 = ChannelViT(size=5*5, hidden_size=768, no_size=15)
        ...
        self.seq_lstm = SeqViT(size=5*5, hidden_size=15, no_size=768)
        self.channel_lstm = ChannelViT(size=5*5, hidden_size=768, no_size=15)
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i <= 3:
                ...
                if i >= 3:
                    x_2 = self.seq_lstms2[i-3](x_1)
                    x_2 = self.channel_lstms2[i-3](x_2)
