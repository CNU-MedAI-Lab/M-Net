...
class SS2D_seq(nn.Module):
    def __init__(
        ...
        self.A_logs = self.A_log_init(self.d_state, self.d_inner, copies=15, merge=True)  # (K=4, D, N)
        self.Ds = self.D_init(self.d_inner, copies=15, merge=True)  # (K=4, D, N)
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        #                      dim=1).view(B, 2, -1, L)
        # xs = torch.cat([x_hwwh, torch.flip(x_hwwh, dims=[-1])], dim=1)  # (b, k, d, l)
        # print(xs.shape, self.x_proj_weight.shape)
        x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs.view(B, K, -1, L), self.x_proj_weight)
        # x_dbl = x_dbl + self.x_proj_bias.view(1, K, -1, 1)
        dts, Bs, Cs = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=2)
        dts = torch.einsum("b k r l, k d r -> b k d l", dts.view(B, K, -1, L), self.dt_projs_weight)
        ...
        # y = out_y[:, 0] + inv_y[:, 0] + wh_y + invwh_y
        # y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, H, W, -1)
        y = self.out_norm(out_y).to(x.dtype)
        # print(y.shape)
        return y
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        xz = self.in_proj(x)
        xz = xz.view(xz.shape[1], xz.shape[2], xz.shape[3], xz.shape[4])
        # print(xz.shape)
        x, z = xz.chunk(2, dim=-1)  # (f, h, w, c)
        # print(x.shape)
        x = x.permute(0, 3, 1, 2).contiguous()
        x = self.act(self.conv2d(x))  # (f, c, h, w)
        y = self.forward_core(x)
        y = y * F.silu(z)
        out = self.out_proj(y)
        if self.dropout is not None:
...
class VSSBlock_channel(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
        ...
        # print(hidden_dim)
        self.ln_1 = norm_layer(hidden_dim)
        self.self_attention = SS2D_channel(d_model=hidden_dim, expand=expand, dropout=attn_drop_rate, d_state=d_state, channels = channels, **kwargs)
        self.drop_path = DropPath(drop_path)
...
class SS2D_channel(nn.Module):
    def __init__(
        ...
        self.dt_projs = []
        for i in range(self.channels):
            self.dt_projs.append(self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                         **factory_kwargs))
        self.dt_projs_weight = nn.Parameter(torch.stack([t.weight for t in self.dt_projs], dim=0))  # (K=4, inner, rank)
        self.dt_projs_bias = nn.Parameter(torch.stack([t.bias for t in self.dt_projs], dim=0))  # (K=4, inner)
        del self.dt_projs

        self.A_logs = self.A_log_init(self.d_state, self.d_inner, copies=self.channels, merge=True)  # (K=4, D, N)
        self.Ds = self.D_init(self.d_inner, copies=self.channels, merge=True)  # (K=4, D, N)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        b, c, h, w, f = x.shape
        # x = x.view(c, h, w, f)
        # x = x.permute(0, 4, 2, 3, 1) # b,c,h,w,f
        xz = self.in_proj(x)
        xz = xz.view(xz.shape[1], xz.shape[2], xz.shape[3], xz.shape[4])
        # print(xz.shape)
        x, z = xz.chunk(2, dim=-1)  # (c, h, w, f)
        # print(x.shape)
        x = x.permute(0, 3, 1, 2).contiguous()
        x = self.act(self.conv2d(x))  # (c, f, h, w)
        y = self.forward_core(x)
        y = y * F.silu(z)
        out = self.out_proj(y)
        if self.dropout is not None:
...
class VSSBlock_seq(nn.Module):
    def __init__(
        ...
        # print(hidden_dim)
        self.ln_1 = norm_layer(hidden_dim)
        self.self_attention = SS2D_seq(d_model=hidden_dim, expand=expand, dropout=attn_drop_rate, d_state=d_state, **kwargs)
        self.drop_path = DropPath(drop_path)
...
class SeqMambaBlock(nn.Module):
    ...
    def forward(self, x):
        # input shape is [b, frames, channels, w, h]
        b,f,h,w,c = x.shape

        xs = self.mamba_f(x) # b, f, h, w, c
        # xs = xs.view(b, c, w, h, f)
        xs = x + xs
        # xs = torch.permute(xs, (0, 1, 4, 2, 3))
        # b, f, c, w, h
        return xs
...
class ChannelMambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        b, f, h, w, c = x.shape
        x = x.permute(0, 4, 2, 3, 1) # b,c,h,w,f
        xs = self.mamba_c(x)  # b, f, h, w, c
        # xs = xs.view(b, c, w, h, f)
        xs = x + xs # b, c, h, w, f
        xs = torch.permute(xs, (0, 4, 2, 3, 1))
        # b, f, c, w, h
        return xs
...
class VSSM(nn.Module):
    def __init__(self, Frames, patch_size=4, in_chans=4, num_classes=3, depths=[2, 2, 9, 2],
        ...
        self.seq_mambas1 = nn.ModuleList()
        self.seq_mambas2 = nn.ModuleList()
        self.seq_mambas3 = nn.ModuleList()
        self.seq_mambas4 = nn.ModuleList()
        self.channel_mamba1 = nn.ModuleList()
        self.channel_mamba2 = nn.ModuleList()
        self.channel_mamba3 = nn.ModuleList()
        for i_layer in range(self.num_layers):
            ...
            if i_layer <=2 :
                mamba_f1 = SeqMambaBlock(
                    channels=int((dims[0] * 2 ** i_layer)*2),
                    drop_path=0.1,
                    attn_drop_rate=0.1,
                    d_state_f=32,
                    d_state_c=8,
                ...
                self.seq_mambas1.append(mamba_f1)
                self.seq_mambas2.append(mamba_f2)
                self.seq_mambas3.append(mamba_f3)
                self.seq_mambas4.append(mamba_f4)
                self.channel_mamba1.append(mamba_c1)
                self.channel_mamba2.append(mamba_c2)
                self.channel_mamba3.append(mamba_c3)
            elif i_layer == 3:
                mamba_f1 = SeqMambaBlock(
                        channels=768,
                        drop_path=0.1,
        ...
        for i_layer in range(self.num_layers):
            ...
            self.se = SELayer(4, 2)
        # self.lstm = ConvLSTM(768, [768, 768, 384, 384, 192, 192, ],
        #                      [(3, 3), (3, 3), (3, 3), (3, 3), (3, 3), (3, 3),],
        #                      6, True)
        # self.lstm2 = ConvLSTM(384, [384, 192, 192, 192, 384],
        #                      [ (3, 3), (3, 3), (3, 3), (3, 3), (3, 3)],
        #                      5, True)
        self.seq_channel_mamba = SeqMambaBlock(
            channels=768,
            drop_path=0.1,
            attn_drop_rate=0.0,
            d_state_f=32,
        ...
        self.channel_mamba = ChannelMambaBlock(
            channels=768,
            drop_path=0.1,
            attn_drop_rate=0.0,
            d_state_f=32,
        ...
        # self.seq_channel_mamba2 = SeqMambaBlock(
        #     channels=768,
        #     drop_path=0.1,
        #     attn_drop_rate=0.0,
        #     d_state_f=8,
        ...
        #
        # )
        # self.u1 = Upsample_block_withouty(192, 384, 3, )
        # self.u2 = Upsample_block_withouty(384, 768, 3, )
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i <=3:
                ...
                x_1 = self.seq_mambas1[i](x)
                x_1 = self.channel_mamba1[i](x_1)
                x_2 = self.seq_mambas2[i](x_1)
                x_2 = self.channel_mamba2[i](x_2)
                x_3 = self.seq_mambas3[i](x_2)
                x_3 = self.channel_mamba3[i](x_3)
                x_4 = self.seq_mambas4[i](x_3)
                # c, h, w, _, _ = x.size()
                ...
                res, idx = self.se(res)
                # res = self.norm(res)
                # # x_3 = x_3.view(x.shape[1], x.shape[2], x.shape[3], x.shape[4])
                # # print(x_0.shape)
                for j in range(4):
                    if idx != j:
                        x = x + (res[j, :, :, :, :])
