...
from .seq_model.xlstm.xlstm2 import mLSTM as xLSTM
# from itertools import pairwise


class LSTM(nn.Module):
    def __init__(self, batch_size, input_dim, head_dim, head_num, p_factor=4/3):
    ...
    def forward(self, x):
        # input is seq, b, dim
        self.hid = self.lstm.init_hidden(self.batch_size)
        outs = []
        for prev in (x):
            # print(i)
            # i += 1
            ...
            # print(prev.shape)
            pred, hid = self.lstm(prev, self.hid)
...
class SeqLSTM(nn.Module):
    def __init__(self, batch_size, input_dim, head_dim=4, head_num=2, p_factor=4/3):
        super(SeqLSTM, self).__init__()
        # need input shape is [seq_len(f), b(c), input_dim(size(hxw))]
        self.lstm = LSTM(batch_size, input_dim, head_dim, head_num)
    ...
    def forward(self, x):
        ...
        x = x.view(x.shape[0], x.shape[1], x.shape[2] * x.shape[3]) # f, c, hxw
        out = self.lstm(x)
        x = out + x # f, c, hxw
        x = x.view(x.shape[0], x.shape[1], h, w) # f, c, h, w
...
class ChannelLSTM(nn.Module):
    def __init__(self, batch_size, input_dim, head_dim=4, head_num=2, p_factor=4/3):
        ...
        # need input shape is [seq_len(c), b(f), size(hxw)]
        self.lstm = LSTM(batch_size, input_dim, head_dim, head_num)
    ...
    def forward(self, x):
        ...
        x = x.view(x.shape[0], x.shape[1], x.shape[2]*x.shape[3]) # c,f, hxw
        out = self.lstm(x)
        x = out + x # c, f, hxw
        x = x.view(x.shape[0], x.shape[1], h, w) # c, f, h, w
...
class VSSM(nn.Module):
    def __init__(self, Frames, patch_size=4, in_chans=4, num_classes=3, depths=[2, 2, 9, 2],
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer <= 2:
                if i_layer <2:
                    ...
                    size = 5
                lstm_s1 = SeqLSTM(batch_size=int((dims[0] * 2 ** i_layer)*2), input_dim=size * size)
                lstm_s2 = SeqLSTM(batch_size=int((dims[0] * 2 ** i_layer)*2), input_dim=size * size)
                # batch_size, input_dim, head_dim, head_num, p_factor=4/3
                # lstm_s3 = SeqLSTM(input_size=size * size, hidden_size=15, num_layers=4)
                # lstm_s4 = SeqLSTM(input_size=size * size, hidden_size=15, num_layers=8)

                lstm_c1 = ChannelLSTM(batch_size=15, input_dim=size * size)
                lstm_c2 = ChannelLSTM(batch_size=15, input_dim=size * size)
                ...
                lstm_s1 = SeqLSTM(batch_size=768, input_dim=5*5)
                lstm_s2 = SeqLSTM(batch_size=768, input_dim=5*5)
                # lstm_s3 = SeqLSTM(input_size=5 * 5, hidden_size=15, num_layers=4)
                # lstm_s4 = SeqLSTM(input_size=5 * 5, hidden_size=15, num_layers=8)

                lstm_c1 = ChannelLSTM(batch_size=15, input_dim=5*5)
                lstm_c2 = ChannelLSTM(batch_size=15, input_dim=5*5)
                # lstm_c3 = ChannelLSTM(input_size=5 * 5, hidden_size=768, num_layers=4)
                # lstm_c4 = ChannelLSTM(input_size=5 * 5, hidden_size=768, num_layers=4)
        ...
        # self.seq_lstm = SeqLSTM(batch_size=768, input_dim=5 * 5)
        # self.channel_lstm =ChannelLSTM(batch_size=15, input_dim=5 * 5)
