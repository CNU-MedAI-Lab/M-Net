...
class SeqConvLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = split_image_to_patches(x, 5).to('cuda') # (c, block_len, f, h, w)
        patches = patches.permute(0, 2, 1, 3, 4) # (c, f, block_len, h, w)
        # print(patches.shape)
        out, _= self.convlstm(patches)
        out = out[0].permute(0, 1, 3, 4, 2) # (c, f, h, w, block_len)
        # out = out[0].permute(0, 2, 1, 3, 4).to('cuda') # c, block_len, f, h, w
        out = self.ln1(out).permute(0, 4, 1, 2, 3) # (c, block_len, f, h, w)
        out = restore_image_from_patches(out, (h, w), 5) # c, f, h, w


        x = out + x  # (c, f, h, w)
        x = x.permute(1, 2, 3, 0)  # f, h, w, c
        return x
...
class ChannelConvLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 1, 2)  # (f, c, h, w)
        patches = split_image_to_patches(x, 5)  # (f, block_len, c, h, w)
        patches = patches.permute(0, 2, 1, 3, 4)  # (f, c, block_len, h, w)
        out, _ = self.convlstm(patches)
        out = out[0].permute(0, 1, 3, 4, 2)# f, c, h, w, block_len
        # out = out[0].permute(0, 2, 1, 3, 4).to('cuda')  # f, block_len, c, h, w
        out = self.ln1(out).permute(0, 4, 1, 2, 3) # f, block_len, c, h, w
        out = restore_image_from_patches(out, (h, w), 5)  # f, c, h, w
        x = out + x  # (f, c, h, w)
        x = x.permute(0, 2, 3, 1)  # f, h, w, c
        return x
...
class VSSM(nn.Module):
    def __init__(self, Frames, patch_size=4, in_chans=4, num_classes=3, depths=[2, 2, 9, 2],
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer <= 2 and i_layer >0:
                if i_layer <2:
                    size = int(20/(i_layer + 1))
                else:
                    size = 5
                dim_t = int((dims[0] * 2 ** i_layer)*2)
                lstm_s1 = SeqConvLSTM(input_dim=(size//5)*(size//5), hidden_dim=(size//5)*(size//5),
                                      kernel_size=(3,3), num_layers=1)
                # lstm_s2 = SeqConvLSTM(input_dim=(size//5)*(size//5), hidden_dim=(size//5)*(size//5),
                #                       kernel_size=(3,3), num_layers=1)
                # lstm_s3 = SeqConvLSTM(input_dim=(size // 5) * (size // 5), hidden_dim=(size // 5) * (size // 5),
                #                       kernel_size=(3,3), num_layers=1)
                # lstm_s4 = SeqConvLSTM(input_dim=(size // 5) * (size // 5), hidden_dim=(size // 5) * (size // 5),
                #                       kernel_size=(3,3), num_layers=1)


                lstm_c1 = ChannelConvLSTM(input_dim=(size//5)*(size//5), hidden_dim=(size//5)*(size//5),
                                          kernel_size=(3,3), num_layers=1)
                # lstm_c2 = ChannelConvLSTM(input_dim=(size//5)*(size//5), hidden_dim=(size//5)*(size//5),
                #                           kernel_size=(3,3), num_layers=1)
                # lstm_c3 = ChannelConvLSTM(input_dim=(size // 5) * (size // 5), hidden_dim=(size // 5) * (size // 5),
                #                           kernel_size=(3,3), num_layers=1)
                # lstm_c4 = ChannelConvLSTM(input_dim=(size // 5) * (size // 5), hidden_dim=(size // 5) * (size // 5),
                #                           kernel_size=(3,3), num_layers=1)
                ...
                lstm_s1 = SeqConvLSTM(input_dim=5//5, hidden_dim=5//5,
                                      kernel_size=(3,3), num_layers=1)
                # lstm_s2 = SeqConvLSTM(input_dim=5//5, hidden_dim=5//5,
                #                       kernel_size=(3,3), num_layers=1)
                # lstm_s3 = SeqConvLSTM(input_dim=5 // 5, hidden_dim=5 // 5,
                #                       kernel_size=(3,3), num_layers=1)
                # lstm_s4 = SeqConvLSTM(input_dim=5 // 5, hidden_dim=5 // 5,
                #                       kernel_size=(3,3), num_layers=1)

                lstm_c1 = ChannelConvLSTM(input_dim=5//5, hidden_dim=5//5,
                                          kernel_size=(3,3), num_layers=1)
                # lstm_c2 = ChannelConvLSTM(input_dim=5//5, hidden_dim=5//5,
                #                           kernel_size=(3,3), num_layers=1)
                # lstm_c3 = ChannelConvLSTM(input_dim=5 // 5, hidden_dim=5 // 5,
                #                           kernel_size=(3,3), num_layers=1)
                # lstm_c4 = ChannelConvLSTM(input_dim=5 // 5, hidden_dim=5 // 5,
                #                           kernel_size=(3,3), num_layers=1)
        ...
        self.seq_lstm = SeqConvLSTM(input_dim=5//5, hidden_dim=5//5,
                                      kernel_size=(3,3), num_layers=1)
        self.channel_lstm = ChannelConvLSTM(input_dim=5//5, hidden_dim=5//5,
                                          kernel_size=(3,3), num_layers=1)
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i <= 3 and i>0:
                ...
                x_1 = self.seq_lstms1[i-1](x)
                x_1 = self.channel_lstms1[i-1](x_1)
                # x_2 = self.seq_lstms2[i-1](x_1)
                # x_2 = self.channel_lstms2[i-1](x_2)
